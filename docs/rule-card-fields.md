...
| `weekday` | list | 승인일 요일. `월`~`일`. 예 `[토, 일]`. **판정을 내리는 카드에는 못 쓴다** (아래 참고) |
...
- `match.weekday`가 있는 카드가 `verdict`를 갖거나 선택지에 `verdict`를 두면 거부합니다.
  요일은 소명 신호일 뿐 판정 근거가 아닙니다 — 요일로 경비를 막는 조문이 없고, 주말 불가를
  두면 토요일 거래처 미팅이 되묻기 없이 불가로 끝납니다. 속성(R-061 `주말결제`)만 싣습니다.
