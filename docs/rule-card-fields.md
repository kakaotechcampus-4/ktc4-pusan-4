...
| `weekday` | list | 승인일 요일. `월`~`일`. 예 `[토, 일]`. **판정은 소명으로 풀리는 불가만** 낼 수 있다 (아래 참고) |
...
**`불가`인데 `questions`가 비어 있지 않으면 소명 대기입니다**(R-311~313 주말 식대). 추정으로
제외했지만 답하면 풀리는 건이라 "제외했어요. 업무였다면 알려주세요"로 보여야 합니다. 엔진은
불가가 확정되면 질문을 지우는데, 불가를 낸 카드의 미응답 질문에 불가를 푸는 선택지가 있으면
확정으로 보지 않고 남깁니다(`JudgmentEngine`, `QuestionSpec.canLiftUnavailable`).
...
- `match.weekday`가 있는 카드의 판정은 **소명으로 풀리는 불가**만 허용합니다. 요일은 조문이
  아니라 추정의 근거라서입니다(요일로 경비를 막는 조문이 없습니다).
  - `verdict`가 `가능`·`확인필요`면 거부합니다. 확인필요는 이미 확정된 거래를 새 정보 없이 끌어내립니다.
  - `verdict: 불가`면 답에 따라 불가가 풀리는 선택지(`verdict`가 불가가 아닌 것)를 가진 질문이
    있어야 합니다. 없으면 "주말 = 무조건 불가"가 되어 토요일 거래처 미팅이 소명 없이 제외됩니다.
  - `verdict`가 없으면 선택지에도 `verdict`를 둘 수 없습니다.
